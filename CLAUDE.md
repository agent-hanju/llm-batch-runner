...
모든 runner(`ChatCompletionsRunner`, `AnthropicBatchRunner`, `CliRunner`)는 동일한 duck-type 인터페이스를 구현한다:
- `stream(requests: list[LLMRequest]) -> Iterator[LLMResponse]` — 완료 순서대로 yield
- `run(req: LLMRequest) -> LLMResponse` — 단건 실행

공통 Protocol/ABC는 없다 — duck typing으로 충분하고 `__main__._build_runner()`가 union type을 반환한다.
...
collector = BatchCollector(runner)
for item in source:
    collector.add(build_request(item))
...
- 각 runner는 `stream(requests)` generator를 구현 — `LLMResponse`를 완료 순서대로 yield
